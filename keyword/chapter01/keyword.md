...
기본 키를 더 자세히 알기 이전에 슈퍼키, 후보키의 정의를 통해 좀 더 명확하게 정의해보겠다.
...
- 즉, **최소성과 유일성**을 만족하는 속성이라고 이해하면 더 쉽게 이해할 수 있다.
...
---
**Entity-Relationship Diagram**의 약자로 엔티티 간의 관계를 시각적으로 나타낸 다이어그램이다.<br>
데이터베이스 설계 과정에서 `개념적 설계 단계`의 산출물이라고 할 수 있다.

데이터베이스 설계 과정을 통해 ER 다이어그램이 어떤 개념인지 더 명확하게 알아보자
...
위 과정을 통해 증명된 사실은 ER 다이어그램은 데이터베이스를 설계하는 데에 있어서 기반이 된다는 것이다.<br>
데이터베이스 설계 과정에서 ER 다이어그램을 통해 저장된 데이터를 설계 이전에 미리 모델링 할 수 있고,
이를 통해 데이터베이스 설계를 효율적으로 진행할 수 있게 된다.<br>
또한, 프로젝트 개발 측면에서도 팀 끼리 사전에 데이터 구조를 명확히 정의하고 공유함으로써 원활한 협업과 일관된 데이터 관리를 가능하게 해준다.
...
- **데이터 중복**: 한 교수가 여러 과목을 담당하거나 여러 학과에 소속될 경우, 동일한 교수 정보가 여러 번 반복되어 저장된다.

- **데이터 무결성 문제**: 데이터 중복으로 인해 한 곳에서 정보가 수정되었지만 다른 곳에서는 수정되지 않는 상황이 발생할 수 있다. 
예를 들어, 교수의 연락처 정보가 변경되었을 때, 모든 관련 레코드를 일일이 수정해야 한다.

- **데이터 삭제 및 갱신의 어려움**: 특정 교수가 퇴사하여 해당 레코드를 삭제하려고 할 때, 그 교수가 담당하던 과목 정보도 함께 삭제될 위험이 있다. 이는 의도치 않은 데이터 손실을 초래할 수 있다.

- **데이터베이스 확장성 저하**: 모든 정보를 하나의 테이블에 저장하면, 새로운 속성이나 관계를 추가할 때 테이블 구조를 변경해야 하므로 확장성이 저하된다.
...
이를 통해 데이터베이스의 효율성, 일관성 및 정확성을 개선할 수 있다.
...
- 비정규화 특징
  ...
  - 비정규화의 주요 특징은 중복된 데이터를 허용한다는 것이다.
  - 이렇게 중복을 허용함으로써 읽기 성능에서 이점이 있다.
...
### 다른 측면, 지연로딩(Lazy Loading)
- 반 정규화는 데이터 중복을 허용하고 조회 성능을 개선하지만, 데이터 일관성 관리에 부담을 줄 수 있다.<br>
- 이를 해결하기 위해 지연 로딩(Lazy Loading) 전략을 사용하면, 데이터를 필요한 시점에만 로드해 메모리 최적화와 일관성 유지가 가능하다.<br>
- 그러나 지연 로딩은 반복 조회 시 N+1 문제나 성능 저하를 일으킬 수 있어서 상황에 맞게 적절히 선택해야 한다.
